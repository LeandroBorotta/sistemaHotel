...
<main class="container-fluid py-5 mb-5 text-light bg-escuro row">
            ...
            <div class="col">
                <div class="card bg-body-tertiary bg-dark h-100 border-warning borde">
                    <div class="d-flex align-items-center justify-content-between border-bottom">
                        ...
                        <i class="bi bi-{{ starsData[hotel.id] }} text-warning mx-3 fs-4" data-hotel-id="{{ hotel.id }}" data-user-id="{{ idUser }}"></i>

                    </div>
                    
                    <div class="card-body">
                        <h5 class="card-title">{{hotel.localizacao}}</h5>
                        <h6 class="card-subtitle">{{hotel.nota}}</h6>
                        <p class="card-text ">
                           {{hotel.descricao}} 
                        </p>
                        <div class="d-flex">
    ...
    <script>
            var favoriteIcons = document.querySelectorAll('.bi-star');
                ...
                if (favoriteIcon.classList.contains('bi-star')) {
                ...
                alert('Botão clicado!');
                var hotelId = favoriteIcon.getAttribute('data-hotel-id');
                var userId = favoriteIcon.getAttribute('data-user-id');
                var isFavorite = favoriteIcon.classList.contains('bi-star-fill');

                $.ajax({
                url: 'http://localhost/exerc%C3%ADciosIndividuais/SimpleRouter3/public/ajax/home',
                method: 'POST',
                data: {
                    item_id: hotelId,
                    user_id: userId,

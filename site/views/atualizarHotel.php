...
<header class="container-fluid bg-dark text-light text-center">
    ...
    <nav class="navbar bg-dark">
            <div class="container">
                ...
                <i class="bi bi-calendar-event fs-1 me-2"></i>
                    Hotel Scheduler - Atualizar Hotel
                </a>
    ...
    <main class="row container d-flex justify-content-center align-items-center vh-120">
        <form action="/exercíciosIndividuais/SimpleRouter3/public/adm/{{hotel[0]['id']}}/atualizar" id="form" method="post" class="border p-3 col-8 ">
            <span class="text-success">{{mensagem}}</span>
            <span class="text-danger">{{erro}}</span>
            <div class="row d-flex flex-column mb-3">
                <label for="nome"> Nome: </label>
                <input class="col-12"  name="nome" type="text" required value="{{ hotel[0]['nome'] }}">
            </div>
            <div class="row d-flex flex-column mb-3">
                <label for="localizacao">Localização:</label>
                <input class="col-12" name="localizacao" required type="text" value="{{ hotel[0]['localizacao'] }}">
            </div>

            <div class="row d-flex flex-column mb-3">
                <label for="preco">Preço por noite:</label>
                <input class="col-12"  name="preco" type="text" required value="{{ hotel[0]['preco'] }}">
            </div>

            <div class="row d-flex flex-column mb-3">
                <label for="descricao">Descrição:</label>
                <textarea class="form-control" name="descricao" value="aaa" required id="descricao" cols="30" rows="5" style="resize: none;">{{ hotel[0]['descricao'] }}</textarea>
            </div>
            ...
            <div class="form-check form-switch">
                <input class="form-check-input" name="banheira" type="checkbox" {% if hotel[0]['banheira'] == 1 %}checked{% endif %} id="flexSwitchCheckDefault">
                <label class="form-check-label" for="flexSwitchCheckDefault">Os quartos possuem banheira?</label>
            </div>

            <div class="form-check form-switch">
                <input class="form-check-input" name="varanda" type="checkbox" {% if hotel[0]['varanda'] == 1 %}checked{% endif %} id="flexSwitchCheckChecked">
                <label class="form-check-label" for="flexSwitchCheckChecked">Os quartos possuem varanda?</label>
            </div>

            <div class="form-check form-switch">
                <input class="form-check-input" name="arCondicionado" type="checkbox" {% if hotel[0]['arCondicionado'] == 1 %}checked{% endif %} id="flexSwitchCheckDisabled">
                <label class="form-check-label" for="flexSwitchCheckDisabled">Os quartos possuem ar condicionado?</label>
            </div>

            <div class="form-check form-switch">
                <input class="form-check-input" name="freezer" type="checkbox" {% if hotel[0]['freezer'] == 1 %}checked{% endif %} id="flexSwitchCheckCheckedDisabled">
                <label class="form-check-label" for="flexSwitchCheckCheckedDisabled">Os quartos possuem freezer?</label>
            </div>

            <div class="form-check form-switch">
                <input class="form-check-input" name="limpezaDiaria" type="checkbox" {% if hotel[0]['limpezaDiaria'] == 1 %}checked{% endif %} id="flexSwitchCheckCheckedDisabled">
                <label class="form-check-label" for="flexSwitchCheckCheckedDisabled">Os quartos possuem limpeza diária?</label>
            </div>


            <div class="form-check form-switch">
                <input class="form-check-input" name="quadraEsportiva" type="checkbox" {% if hotel[0]['quadraEsportiva'] == 1 %}checked{% endif %} id="flexSwitchCheckDisabled">
                <label class="form-check-label" for="flexSwitchCheckDisabled">O hotel tem quadras esportivas?</label>
            </div>

            <div class="form-check form-switch">
                <input class="form-check-input" name="piscina" type="checkbox" {% if hotel[0]['piscina'] == 1 %}checked{% endif %} id="flexSwitchCheckCheckedDisabled">
